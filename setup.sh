...
echo "export HOST_DOM=$Address" >> ./EX1/start.sh
echo "export DB_PORT=3306" >> ./EX1/start.sh
echo "export DB_PASSWORD=$PASSWORD" >> ./EX1/start.sh
echo "export DB_USERNAME=$USERNAME" >> ./EX1/start.sh
echo "node app.js" >> ./EX1/start.sh
...
aws ec2 authorize-security-group-ingress        \
    ...
    --cidr $MY_IP/32
echo "setup rule allowing HTTP (port 80) accsess to every one"
aws ec2 authorize-secruoty-group-ingress --group-name  $SEC_GRP --port 80 --cidr 0.0.0.0/0
aws ec2 authorize-secruoty-group-ingress --group-name  $SEC_GRP --port 80 --cidr ::/0$
